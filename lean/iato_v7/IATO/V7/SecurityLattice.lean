...
theorem le_EL2_iff (L : SecurityLevel) : L ≤ EL2 ↔ L = EL0 ∨ L = EL1 ∨ L = EL2 := by
  cases L <;> simp [le_def, rank]

/-- Every security level is below or equal to EL3. -/
theorem le_EL3_iff (L : SecurityLevel) : L ≤ EL3 ↔ L = EL0 ∨ L = EL1 ∨ L = EL2 ∨ L = EL3 := by
  cases L <;> simp [le_def, rank]

/-- Levels greater than or equal to EL2 are exactly EL2 and EL3. -/
theorem EL2_le_iff (L : SecurityLevel) : EL2 ≤ L ↔ L = EL2 ∨ L = EL3 := by
  cases L <;> simp [le_def, rank]

/-- Levels greater than or equal to EL1 are exactly EL1, EL2, and EL3. -/
theorem EL1_le_iff (L : SecurityLevel) : EL1 ≤ L ↔ L = EL1 ∨ L = EL2 ∨ L = EL3 := by
  cases L <;> simp [le_def, rank]
...
theorem no_downgrade {H L : SecurityLevel} (hHL : H > L) : ¬flows_to H L := by
  intro h
  exact (ne_of_gt hHL) (SecurityLevel.le_antisymm h (Nat.le_of_lt hHL))
